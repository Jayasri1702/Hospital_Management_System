...
const BookingPage = () => {
  ...
  const getUserData = async () => {
    try {
      const res = await axios.post("/HospitalManagement/getDoctorById",{ doctorId: params.id },
        {
          headers: {
  ...
  const handleAvailability = async () => {
    try {
      ...
      const res = await axios.post("/HospitalManagement/booking-availability",{ doctorId: params.doctorId, date, time },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const handleBooking = async () => {
    try {
      ...
      dispatch(showLoading());
      console.log(user)
      const res = await axios.post("/HospitalManagement/book-appointment",
        {
