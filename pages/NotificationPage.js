...
function NotificationPage() {
    ...
    const handleMarkAllRead = async()=>{
        try{
            ...
            const res=await axios.post('/HospitalManagement/get-all-notification',{
                userId:user._id
            },{
                headers:{
    ...
    const handleDeleteAllRead =async()=>{
        try{
            dispatch(showLoading())
            const res = await axios.post('./HospitalManagement/delete-all-notification',{userId:user._id},{
                headers:{
                    Authorization:`Bearer ${localStorage.getItem('token')}`
                }
            })
