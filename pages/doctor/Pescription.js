...
const Pescription = () => {
    ...
    const getAppointments = async () => {
        try {
            const res = await axios.post("/HospitalManagement/getAppointmentById",{ appointmentId: params.id },
                {
                headers: {
    ...
    const onfinishHandler = async(values)=>{
        try{
        const res = await axios.post('/HospitalManagement/postpescription',{...values},
            {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("token"),
                },
